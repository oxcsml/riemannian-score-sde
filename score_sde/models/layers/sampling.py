...
class Conv2d(hk.Module):
    ...
    def __call__(self, x):
        ...
        w = hk.get_parameter(
            "w",
            shape=(self.kernel, self.kernel, x.shape[-1], self.fmaps),
            dtype=x.dtype,
            init=self.kernel_init,
        )

        if self.up:
            x = conv_upsample_2d(x, w, data_format="NHWC", k=self.resample_kernel)
        elif self.down:
...
def conv_upsample_2d(x, w, k=None, factor=2, gain=1, data_format="NHWC"):
