    ...
    if color is None:
        ...
        if color_palette.shape[0] == 3:
            color_palette = np.vstack(
                (color_palette, np.ones((1, color_palette.shape[1])))
            )
        colors = np.vstack(
            (
    ...
    for vert, row in zip(bm.verts, vector_field):
        ...
        if row.size == 12:
            ...
            if np.all(row[3:6] == 0):
                ...
                axis = (
                    np.array((0, 0, 1))
                    if np.allclose(arrow, np.array((1, 0, 0)))
                    or np.allclose(arrow, np.array((-1, 0, 0)))
                    else np.array((1, 0, 0))
                )
...
def add_vector_field(vec_bm, arr_obj, scale=1, name="Vector Field"):
    vf_mesh = bpy.data.meshes.new(name)
    vf_obj = bpy.data.objects.new(name, vf_mesh)
    vec_bm.to_mesh(vf_mesh)
    bpy.context.scene.collection.objects.link(vf_obj)

    mod = vf_obj.modifiers.new(name, "NODES")
    ...
    align_rotation_normal_node = mod.node_group.nodes.new(
    ...
    mod.node_group.links.new(
        input_node.outputs["Geometry"], arrow_length_node.inputs["Geometry"]
    )
    mod.node_group.links.new(
        arrow_length_node.outputs["Geometry"], normal_x_length_node.inputs["Geometry"]
    )
    mod.node_group.links.new(
        normal_x_length_node.outputs["Geometry"],
        normal_z_length_node.inputs["Geometry"],
    )
    mod.node_group.links.new(
        normal_z_length_node.outputs["Geometry"], combine_node.inputs["Geometry"]
    )
    mod.node_group.links.new(
        combine_node.outputs["Geometry"], arrow_scale_node.inputs["Geometry"]
    )
    mod.node_group.links.new(
        arrow_scale_node.outputs["Geometry"], constant_scale_node.inputs["Geometry"]
    )
    mod.node_group.links.new(
        constant_scale_node.outputs["Geometry"],
        align_rotation_normal_node.inputs["Geometry"],
    )
    mod.node_group.links.new(
        align_rotation_normal_node.outputs["Geometry"],
        align_rotation_node.inputs["Geometry"],
    )
    mod.node_group.links.new(
        align_rotation_node.outputs["Geometry"], point_instance_node.inputs["Geometry"]
    )
    mod.node_group.links.new(
        point_instance_node.outputs["Geometry"], output_node.inputs["Geometry"]
    )
...
def create_vector_arrow(
    length=10, ratio=0.1, vertices=25, color=(0, 0, 0, 1), name="Arrow"
):
    bm = bmesh.new()
    prod = np.pi**2 * ratio**2 / length**2
    shift = (prod - 2 * np.sqrt(-prod * ratio**2 + prod + 4 * ratio**2)) / (
        prod - 4
    )
    scale = 1 - shift
...
def create_dot(
    location=(0, 0, 0), radius=1, color=(0, 0, 0, 1), subdivisions=5, name="Dot"
):
    bm = bmesh.new()
    bmesh.ops.create_icosphere(bm, subdivisions=subdivisions, diameter=radius * 2)
...
def create_plane(location=(0, 0, 0), scale=(1, 1, 1), color=(0, 0, 0, 1), name="Plane"):
...
def create_linear_to_srgb_bw_node_group():
    group = bpy.data.node_groups.new(
        type="CompositorNodeTree", name="Linear to sRGB BW"
    )
    ...
    group.links.new(
        lower_first_multiply.outputs["Value"], lower_second_multiply.inputs[0]
    )
...
def create_srgb_to_linear_bw_node_group():
    group = bpy.data.node_groups.new(
        type="CompositorNodeTree", name="sRGB to Linear BW"
    )
    group.inputs.new("NodeSocketFloat", "Value")
    group.outputs.new("NodeSocketFloat", "Value")
...
def create_linear_to_srgb_color_node_group():
    group = bpy.data.node_groups.new(
        type="CompositorNodeTree", name="Linear to sRGB Color"
    )
...
def create_srgb_to_linear_color_node_group():
    group = bpy.data.node_groups.new(
        type="CompositorNodeTree", name="sRGB to Linear Color"
    )
    group.inputs.new("NodeSocketColor", "Image")
    group.outputs.new("NodeSocketColor", "Image")
...
def setup_compositor(
    mask_center=(0.5, 0.15),
    mask_size=(0.95, 0.2),
    mask_blur_size=32,
    shadow_color_correction_exponent=1,
):
    scene = bpy.context.scene
    ...
    shadow_color_correction_node.inputs[
        1
    ].default_value = shadow_color_correction_exponent
    ...
    scene.node_tree.links.new(
        ratio_node.outputs["Image"], linear_to_srgb_node.inputs["Image"]
    )
    scene.node_tree.links.new(ratio_node.outputs["Image"], invert_node.inputs["Color"])

    scene.node_tree.links.new(
        invert_node.outputs["Color"], rgb_to_bw_node.inputs["Image"]
    )
    ...
    scene.node_tree.links.new(
        rgb_to_bw_node.outputs["Val"], shadow_color_correction_node.inputs[0]
    )
    scene.node_tree.links.new(
        shadow_color_correction_node.outputs["Value"], mask_multiply_node.inputs[0]
    )
    scene.node_tree.links.new(
        mask_multiply_node.outputs["Value"], set_alpha_node.inputs["Alpha"]
    )

    scene.node_tree.links.new(
        ellipse_mask_node.outputs["Mask"], blur_node.inputs["Image"]
    )
    scene.node_tree.links.new(blur_node.outputs["Image"], mask_multiply_node.inputs[1])

    scene.node_tree.links.new(
        linear_to_srgb_node.outputs["Image"], subtract_node.inputs[1]
    )
    ...
    scene.node_tree.links.new(
        divide_node.outputs["Image"], set_alpha_node.inputs["Image"]
    )

    scene.node_tree.links.new(
        set_alpha_node.outputs["Image"], alpha_convert_node.inputs["Image"]
    )
    scene.node_tree.links.new(
        alpha_convert_node.outputs["Image"], srgb_to_linear_node.inputs["Image"]
    )
    scene.node_tree.links.new(
        srgb_to_linear_node.outputs["Image"], alpha_over_node.inputs[1]
    )
    ...
    scene.node_tree.links.new(
        alpha_over_node.outputs["Image"], viewer_node.inputs["Image"]
    )
...
def setup_camera(
    offset=(0, 0, 0), distance=1, angle=(0, 0, 0), lens=85, height=640, crop=None
):
...
def create_poisson_disk_samples(obj, distance_min=0.1):
    ...
    make_geometry_real_workaround_node = mod.node_group.nodes.new(
        "GeometryNodeAttributeMath"
    )
    mod.node_group.links.clear()
    mod.node_group.links.new(
        input_node.outputs["Geometry"], point_distribute_node.inputs["Geometry"]
    )
    mod.node_group.links.new(
        point_distribute_node.outputs["Geometry"],
        point_instance_node.inputs["Geometry"],
    )
    mod.node_group.links.new(
        point_instance_node.outputs["Geometry"],
        make_geometry_real_workaround_node.inputs["Geometry"],
    )
    mod.node_group.links.new(
        make_geometry_real_workaround_node.outputs["Geometry"],
