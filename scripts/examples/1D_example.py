...
from score_sde.sampling import get_pc_sampler, get_predictor, get_corrector
...
#     lambda x, t: (1000000000000000.0) * ScoreFunctionWrapper(MLP(hidden_shapes=3*[128], output_shape=1, act='sin'))(x, t)
# )
score_model = hk.transform_with_state(
...
sns.kdeplot(posterior_samples[0][0,:,0], color='tab:blue')
# sns.kdeplot(target_samples[0,0,:,0], color='tab:orange')
...
plt.plot(x[0,:,0], prior_likelihood)
plt.plot(x[0,:,0], pushforward_likelihood[0])
sns.kdeplot(posterior_samples[0][0,:,0], color='tab:green')


# %%
from scipy.stats import gaussian_kde

kernel = gaussian_kde(posterior_samples[0][0,:,:])

plt.plot(kernel(x))

# %%
import seaborn as sns
samples = jnp.concatenate([next(dataset)[0,:,0] for i in range(10)], axis=0)
