...
def plot(x, ys, dim_0_names, dim_1_names, out):
    K, J, _ = ys.shape
    fig, axis = plt.subplots(nrows=1, ncols=K, figsize=(12, 5), sharex=True, sharey=False)
...
def test_s2_normalisation():
    ...
    for t in ts:
        ...
        plot_and_save2([], pdf=partial(kernel, s=t), out=f"images/s2_pdf/hk_{t:.3f}.jpg")
...
def test_s1():
    ...
    x0 = jnp.expand_dims(jnp.array([1., 0.]), 0)
    
    kernel = partial(heat_kernel, manifold=S1)

    # ts = [0.002, 0.005, 0.01, 0.02, 0.05]
    ts = [0.001, 0.01, 0.1, 1., 5., 10.]
    eps = jnp.pi * 3 / 4
    # ts = [0.1, 0.5, 2., 5., 10.]
    # eps = 0.
    N = 500
    ...
    probs_trunc_1 = jnp.array([kernel(x=x0, y=x, s=t, tol=0., n_max=0) for t in ts])
    probs_trunc_3 = jnp.array([kernel(x=x0, y=x, s=t, tol=0., n_max=1) for t in ts])
    probs_trunc_10 = jnp.array([kernel(x=x0, y=x, s=t, tol=0., n_max=5) for t in ts])
    ...
    probs = jnp.concatenate(list(map(lambda x: jnp.expand_dims(x, 1), [probs_trunc_1, probs_trunc_3, probs_trunc_10, probs_samples, probs_dev])), axis=1)
    dim_1_names = ['truncated(1)', 'truncated(3)', 'truncated(10)', 'samples', 'developement']
