...
def test_score():
    ...
    dataset = DiracDataset([batch_size], mu=jnp.array([1., 0., 0.]))
...
def EulerMaruyama(previous_x, traj, dt, timesteps, f_sde, sde, forward=True):
    ...
    def body(step, val):
        ...
        rng, z = f_sde.manifold.random_normal_tangent(state=rng, base_point=x, n_samples=x.shape[0])
...
def forward_or_backward_process(forward=False):
    ...
    if forward:
        ...
        previous_x = Brownian(S2, T=sde.T, N=sde.N).prior_sampling(jax.random.PRNGKey(0), [n_samples])
    ...
    plot_and_save_video3(traj_x, jnp.exp(traj_logp), traj_logp_grad, timesteps, size=20, fps=10, dpi=50, out=f"images/{name}.mp4",)
...
def score_ambiant(x, t, sde):
    return sde.manifold.to_tangent(ScoreFunctionWrapper(MLP(hidden_shapes=3*[512], output_shape=3, act='sin'))(x, t), x)  # NOTE: regularize orthogonal component?
...
def main(train=False):
    ...
    sde = Brownian(manifold, T=3, N=100, beta_min=1., beta_max=1.)
    batch_size = 512

    mu = jnp.array([[1., 0., 0.]])
    mub = jnp.repeat(jnp.expand_dims(mu, 0), batch_size, 0)
    kappa = 15
    dataset_init = vMFDataset([batch_size], jax.random.PRNGKey(0), manifold, mu=mu.reshape(3), kappa=kappa)
    ...
    plot_and_save3(x0, x, jnp.exp(logp), logp_grad, out=f"images/s2_xt_forw_true_{name}.jpg")
    ...
    x, _ = sampler(next_rng, train_state, t=t)
    # prob = jax.vmap(sde.marginal_log_prob)(x0, x, jnp.ones(x.shape[0]) * t)
    likelihood_fn = get_likelihood_fn(sde, score_model, hutchinson_type='None', bits_per_dimension=False, eps=1e-3)
