...
from score_sde.utils.vis import remove_background, sphere_plot


def plot(
    traj, size=10, dpi=300, out="out", color="red"
):
    ...
    # alpha = np.flip(alpha)
    ax.scatter(traj[-1, :, 0], traj[-1, :, 1], traj[-1, :, 2], s=100, marker="*", color=colours[0])
    for k in range(K):
        c = sns.cubehelix_palette(n_colors=N, reverse=False, as_cmap=False)
        for n in range(N-1):
            ax.plot(traj[n:n+2, k, 0], traj[n:n+2, k, 1], traj[n:n+2, k, 2], lw=1., linestyle='-',
            # alpha = alpha[n],
            alpha = 0.9,
            # color=colours[k])
            color=c[n])
    for i in range(1):
        ax.scatter(traj[-1, :, 0], traj[-1, :, 1], traj[-1, :, 2], s=100, marker="o", alpha=1, color=colours[-1])
    print("save")
    fig.tight_layout()
    plt.savefig(out + ".jpg", dpi=dpi, bbox_inches="tight", transparent=True, pad_inches=-2.1)
    plt.close(fig)
    return fig


def make_animation(
    traj, size=20, dpi=300, fps=10, out="out"
):
    ratio = size / 10
    fig = plt.figure(figsize=(size, size))
    ...
    with writer.saving(fig, out, dpi=dpi):
        ax.scatter(traj[0, :, 0], traj[0, :, 1], traj[0, :, 2], s=ratio*500, marker="X", color=colours[0])
        writer.grab_frame()
        for n in range(N-1):
            for k in range(K):
                ax.plot(traj[n:n+2, k, 0], traj[n:n+2, k, 1], traj[n:n+2, k, 2], lw=ratio*4., linestyle='-',
                # alpha = alpha[n],
                alpha = 0.9,
                # color=colours[k])
                color=c[n])
            writer.grab_frame()
        ax.scatter(traj[-1, :, 0], traj[-1, :, 1], traj[-1, :, 2], s=ratio*500, marker="o", alpha=1, color=colours[-1])
        writer.grab_frame()
        for _ in range(20):
            writer.grab_frame()
        writer.finish()
...
def test_s2():
    ...
    kappa = 200
    dataset = vMFDataset(
        [K], jax.random.PRNGKey(0), S2, mu=mu.reshape(3), kappa=kappa
    )
    ...
    make_animation(traj, dpi=100, size=10, fps=100, out="images/traj_plot.gif")
